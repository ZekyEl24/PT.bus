{{-- File: resources/views/admin/banner/edit.blade.php --}}
<div id="modalEditBanner_{{ $banner->id }}"
    ...
    <div id="modalContentEditBanner_{{ $banner->id }}"
        ...
        <div class="py-6 border-b border-gray-100 text-center relative">
            ...
            <button type="button" onclick="closeModal('modalEditBanner_{{ $banner->id }}', 'formEditBanner_{{ $banner->id }}')"
        ...
        <form id="formEditBanner_{{ $banner->id }}" action="{{ route('banner.update', $banner->id) }}" method="POST" enctype="multipart/form-data">
            ...
            @method('PUT')
            <div class="p-8">
                {{-- HIDDEN INPUT UNTUK ERROR HANDLER JS --}}
            ...
            <div class="flex flex-col md:flex-row gap-8">
                ...
                <div class="flex-1">
                    <label class="block text-xs font-bold text-gray-700 mb-2">Unggah Foto (Biarkan kosong jika tidak ganti)</label>
                    <div class="relative group">
                        ...
                        <input type="file" id="inputGambarEdit_{{ $banner->id }}" name="gambar" accept="image/*" class="hidden"
                            onchange="previewFile('Edit_{{ $banner->id }}')">
                    </div>
                    @error('gambar', 'edit')
                        <p class="text-red-500 text-[10px] mt-1 italic text-center">{{ $message }}</p>
                    @enderror
            ...
            <div class="flex justify-between gap-4 pt-10">
                <button type="button"
                    onclick="closeModal('modalEditBanner_{{ $banner->id }}', 'formEditBanner_{{ $banner->id }}')"
                    class="px-10 py-3 border border-gray-300 text-gray-600 rounded-xl text-xs font-bold hover:bg-gray-50 transition">
                    Batal
                </button>

                <button type="submit"
                    class="px-10 py-3 bg-birua text-white rounded-xl text-xs font-bold hover:bg-biruc transition shadow-lg">
                    Simpan
                </button>
            </div>
            </div>
